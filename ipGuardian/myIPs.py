...
@handle_error
async def address_setting(update, context):
    # user_detail = update.effective_chat
    query = update.callback_query
    address_id = int(query.data.replace('address_setting_', ''))
    ...
    keyboard = [
        [InlineKeyboardButton(await ft_instance.find_keyboard('country_config'), callback_data=f'country_notification_config_{address_id}')]
    ]
    await query.edit_message_text(text=text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='html')


@handle_error
async def country_notification_config(update, context, address_id=None):
    user_detail = update.effective_chat
    query = update.callback_query
...
async def country_ping_notification(update, context):
    ...
    try:
        ...
        return await country_notification_config(update, context, address_id=address_id)
