    ...
    async def get_eligible_notification(self, period_min, refresh=False):
        ...
        if period_min not in get_instance.copy_objects or refresh or self.force_refresh:
            fetch_from_db = posgres_manager.execute(
                'query', {
                    'query': """
                    SELECT subquery.address,subquery.userID,LOWER(co.country_short_name) FROM Country co JOIN (
                        SELECT ac.countryID, ad.address, ad.userID 
                        ...
                        WHERE an.wait_for_check = %s AND ad.status = %s AND ac.status = %s AND an.period_check_in_min = %s 
                    ) subquery ON co.countryID = subquery.countryID""", 'params': (True, True, True, period_min)})

            result_dict = {}
            for index, (domain, user_id, country) in enumerate(fetch_from_db):
